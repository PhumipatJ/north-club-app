...
const EventList = ({ id, clubName }) => {
  ...
  const getPlatformName = (url) => {
    ...
    const regex =
      /(youtube\.com|youtu\.be|facebook\.com|twitter\.com|instagram\.com|linkedin\.com|tiktok\.com)/i;
    let match = null;
    try {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md p-4 flex gap-4">
      ...
      <div className="flex-shrink-0">
        <img
          src={`${
            supabase.storage.from("club-avatars").getPublicUrl(eventData.poster)
              .data.publicUrl
          }`}
          alt="กิจกรรม"
      ...
      <div className="flex flex-col justify-between flex-grow">
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            <span>
              สถานที่จัดกิจกรรม : {getPlatformName(eventData.location)}
            </span>
          </div>
          <div className="flex items-center gap-2 mt-4">
            ...
            <p>
              {" "}
              เวลา : {formatDate(eventData.start_date)} {eventData.start_time} -{" "}
              {eventData.end_time} น.
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-end mt-auto">
          {eventData && eventData.start_date && eventData.end_time ? (
            isEventExpired(eventData.start_date, eventData.end_time) ? (
              ...
              <button className="bg-[#7CE9BF] text-white px-4 py-2 rounded-lg">
                สนใจ
              </button>
            )
          ) : (
        ...
        {isModalOpen && (
          <>
            <div className="fixed inset-0 bg-black/30 flex justify-center items-center z-50">
              <div className="max-w-4xl mx-auto h-[85%] shadow-lg rounded-xl border border-gray-200 bg-white mt-24 mb-8 overflow-y-auto">
                  ...
                  <div className="flex flex-wrap md:flex-nowrap gap-4">
                    <div className="flex items-center justify-center relative overflow-hidden rounded-lg w-full md:w-2/5 aspect-[4/5]">
                      <img
                        src={`${
                          supabase.storage
                            .from("club-avatars")
                            .getPublicUrl(eventData.poster).data.publicUrl
                        }`}
                        alt={eventData.title}
                    ...
                    <div className="flex-1 px-4 overflow-auto">
                      ...
                      <p className="text-gray-600 text-sm">
                        Organized by KMUTNB Photo Club
                      </p>
                      <div className="mt-2 space-y-2 text-gray-700 text-sm ">
                        <h2 className="text-xl font-semibold text-[#FF7E69] mt-4">
                          สถานที่จัดกิจกรรม
                        </h2>
                        <div className="flex">
                          <MapPin size={16} />
                        ...
                        <div className="flex">
                          <Calendar size={16} />
                          <span className="ml-2">
                            {eventData.start_time} - {eventData.end_time} น.
                          </span>
                        </div>
                        ...
                        <a
                          href={`${
                            supabase.storage
                              .from("club-documents")
                              .getPublicUrl(eventData.document).data.publicUrl
                          }`}
                          target="_blank"
