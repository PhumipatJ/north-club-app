...
const Clubpage = () => {
  ...
  if (!club) {
    return <p className="text-center text-red-500">Club not found</p>;
  }
    ...
    <div className="bg-gray-50">
        ...
        <div className="flex flex-col">
          <div className="flex flex-row p-6 h-fit">
            <div className="relative rounded-full flex items-center justify-center mx-12">
              <img src={club.avatar} alt={club.name} className="w-48 h-48 rounded-full -translate-y-1/2" />
            </div>
            <div className="flex flex-col h-fit">
              <h1 className="text-5xl font-bold text-left">{club.name}</h1>
              <div className="text-gray-500 text-left">
                <p>สร้างเมื่อ: {club.createdDate}</p>
                <p>ที่ตั้งชมรม: {club.locate}</p>
                <div className="flex items-center"> {/* Container for email */}
                  <Mail className="w-5 h-5 text-[#7CE9BF]"/>
                  <p className="px-4">{club.socialLinks.email}</p>
                </div>
                <div className="flex items-center"> {/* Container for Facebook */}
                  <FaFacebook className="w-5 h-5 text-[#7CE9BF]"/>
                  <p className="px-4">{club.socialLinks.facebook}</p>
                </div>
                <div className="flex items-center"> {/* Container for Instagram */}
                  <FaSquareInstagram className="w-5 h-5 text-[#7CE9BF]" />
                  <a href="https://www.instagram.com/l_uod_l_/" target="_blank" className="px-4">{club.socialLinks.instagram}</a>
                </div>
              </div>
            </div>
      ...
      <div className=" shadow-md">
        ...
        <div div className="p-6 mt-8">
          <h2 className="text-xl font-semibold">Members ({club.members.length} คน)</h2>
